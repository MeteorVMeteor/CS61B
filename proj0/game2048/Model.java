...
public class Model extends Observable {
    ...
    private static int[][] arr = new int[][]{{0,1},{0,-1},{1,0},{-1,0}};
    ...
    public boolean tilt(Side side) {
        ...
        for(int i = 0; i < size; i++){
            for(int j = size-1, pointer = size-1, max = size; j >= 0; j--){
                ...
                if(checkNeirbor(t, dir, board, max)){
                    ...
                    while(row < max){
                        row += 1;
                        Tile tmp = board.tile(i, row);
                        if(tmp != null && tmp.value() == t.value()){
                            board.move(i, row, t);
                            max = row;
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        // TODO: Fill in this function.
        if(emptySpaceExists(b) || sameValueNeighbor(b)) {
            return true;
        }
        return false;
    }

    public static boolean sameValueNeighbor(Board b){
        int size = b.size();
        for(int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                Tile tmp = b.tile(i, j);
                if(tmp != null && check(tmp, b)){
                    return true;
                }
            }
        }
